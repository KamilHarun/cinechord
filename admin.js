...
async function viewMessage(id, name, email, message) {
    // ID tırnaq içində gəldiyi üçün burada da dəyişiklik etdik.
...
    
    try { 
        await authFetch(`${API.CONTACTS}/${messageId}/read`, { method: 'PATCH' }); 
        loadMessages(); 
        loadDashboard();
    } catch (e) {}
}
...
async function deleteMessage(id) {
    // 1. ID yoxlanılması və rəqəmə çevrilməsi
    const messageId = parseInt(id);
    
    if (isNaN(messageId) || !messageId) {
        console.error("Etibarsız Mesaj ID-si alındı:", id);
...
    if(r.isConfirmed) {
        try {
            // 2. Təmizlənmiş rəqəmi göndəririk
            const res = await authFetch(`${API.CONTACTS}/${messageId}`, { method: 'DELETE' }); 
            
            if(res && res.ok) {
                Swal.fire('Uğurlu!', 'Mesaj Silindi', 'success');
                loadMessages();
                loadDashboard();
            } else {
                Swal.fire('Xəta', `Silinmədi. Server xətası (Status: ${res?.status || 'Bilinmir'}).`, 'error');
            }
        } catch(e) {
            Swal.fire('Xəta', 'Şəbəkə xətası', 'error');
        }
    }
}
